...
@synthesize recordedFile,recorder;
//音频
...
//        self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height-kNewsToolBarHeight, self.frame.size.width, kNewsToolBarHeight);
        ...
        [shareButton setImage:[UIImage imageNamed:@"record-returnbtn.png"] forState:UIControlStateNormal];
        ...
        [playButton setImage:[UIImage imageNamed:@"record-playbtn.png"] forState:UIControlStateNormal];
...
- (void)playAudio:(id *)sender
{
    ...
    if (self.isRecording==YES) {
        ...
        //保存
        NSString* fileName =  [[[recordedFile path] lastPathComponent] stringByDeletingPathExtension];
        
        NSString *mp3url = [NSString stringWithFormat:@"%@/%@.mp3", DOCUMENTS_FOLDER,fileName];//[NSHomeDirectory()
    ...
    if ([self isPlaying]) {
        ...
        if (self.URL==nil) {
            return;
//            NSURL*voiceurl= [NSURL fileURLWithPath:mp3url];
//            if (voiceurl) {
//                //本地
//                
...
-(void)showStopButton
{
    ...
    [playButton setImage:[UIImage imageNamed:@"record-pause.png"] forState:UIControlStateNormal];
}
...
- (void)popAction:(id)sender{
    ...
    if (viewController) {
        ...
        theAnimation1.subtype = kCATransitionFromRight;//{kCATransitionFromLeft, kCATransitionFromRight, kCATransitionFromTop, kCATransitionFromBottom};
        ...
        [viewController.toolBar.layer addAnimation:theAnimation1 forKey:@"animateLayer"];
    }
        ...
        if (self.URL==nil) {
          //  NSURL*voiceurl= [NSURL fileURLWithPath:mp3url];
            if (recordedFile) {
                ...
                [self setupAVPlayerForURL:recordedFile];
